package yamlenv

import (
	"testing"
	"time"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/require"
)

// These tests are designed to run in isolation to avoid issues with the global koanf instance
// Each test should be run separately to ensure clean state

// Test environment variable functionality in isolation
func TestLoadConfig_EnvironmentVariables_Isolated(t *testing.T) {
	baseYAML := `
app:
  name: testapp
  port: 8080
  debug: false
db:
  host: localhost
  port: 5432
version: "1.0.0"
`

	baseFile := createTempYAML(t, baseYAML)

	// Set environment variables
	setEnvVar(t, "ENVTEST_APP__NAME", "envapp")
	setEnvVar(t, "ENVTEST_APP__PORT", "9000")
	setEnvVar(t, "ENVTEST_DB__HOST", "env-db.example.com")
	setEnvVar(t, "ENVTEST_VERSION", "2.0.0")

	var cfg TestConfig
	err := LoadConfig(LoaderOptions{
		BaseFile:  baseFile,
		EnvPrefix: "ENVTEST_",
		Delimiter: "__",
		Target:    &cfg,
	})

	require.NoError(t, err)
	// Environment overrides applied
	assert.Equal(t, "envapp", cfg.App.Name)
	assert.Equal(t, 9000, cfg.App.Port)
	assert.Equal(t, "env-db.example.com", cfg.DB.Host)
	assert.Equal(t, "2.0.0", cfg.Version)

	// Base values preserved where no env override
	assert.False(t, cfg.App.Debug)
	assert.Equal(t, 5432, cfg.DB.Port)
}

// Test priority order in isolation
func TestLoadConfig_PriorityOrder_Isolated(t *testing.T) {
	baseYAML := `
app:
  name: baseapp
  port: 8080
  debug: false
version: "1.0.0"
`

	localYAML := `
app:
  name: localapp
  port: 3000
version: "1.1.0"
`

	baseFile := createTempYAML(t, baseYAML)
	localFile := createTempYAML(t, localYAML)

	// Environment should override both
	setEnvVar(t, "PRIORITY_APP__NAME", "envapp")
	setEnvVar(t, "PRIORITY_VERSION", "2.0.0")

	var cfg TestConfig
	err := LoadConfig(LoaderOptions{
		BaseFile:  baseFile,
		LocalFile: localFile,
		EnvPrefix: "PRIORITY_",
		Delimiter: "__",
		Target:    &cfg,
	})

	require.NoError(t, err)
	// Environment has highest priority
	assert.Equal(t, "envapp", cfg.App.Name)
	assert.Equal(t, "2.0.0", cfg.Version)

	// Local overrides base where no env override
	assert.Equal(t, 3000, cfg.App.Port)

	// Base value where no local or env override
	assert.False(t, cfg.App.Debug)
}

// Test different delimiters in isolation
func TestLoadConfig_DifferentDelimiters_Isolated(t *testing.T) {
	baseYAML := `
app:
  name: testapp
  port: 8080
db:
  host: localhost
`

	baseFile := createTempYAML(t, baseYAML)

	// Test with different delimiter
	setEnvVar(t, "DELIM_APP_NAME", "delimapp")
	setEnvVar(t, "DELIM_DB_HOST", "delim-db.local")

	var cfg TestConfig
	err := LoadConfig(LoaderOptions{
		BaseFile:  baseFile,
		EnvPrefix: "DELIM_",
		Delimiter: "_",
		Target:    &cfg,
	})

	require.NoError(t, err)
	assert.Equal(t, "delimapp", cfg.App.Name)
	assert.Equal(t, "delim-db.local", cfg.DB.Host)
	assert.Equal(t, 8080, cfg.App.Port) // Not overridden
}

// Test no environment prefix in isolation
func TestLoadConfig_NoEnvPrefix_Isolated(t *testing.T) {
	baseYAML := `
app:
  name: testapp
  port: 8080
`

	baseFile := createTempYAML(t, baseYAML)

	// Set env vars without prefix
	setEnvVar(t, "APP__NAME", "noprefix")
	setEnvVar(t, "APP__PORT", "9000")

	var cfg TestConfig
	err := LoadConfig(LoaderOptions{
		BaseFile:  baseFile,
		EnvPrefix: "", // No prefix
		Delimiter: "__",
		Target:    &cfg,
	})

	require.NoError(t, err)
	assert.Equal(t, "noprefix", cfg.App.Name)
	assert.Equal(t, 9000, cfg.App.Port)
}

// Test complex nested structure with environment overrides in isolation
func TestLoadConfig_ComplexNestedStructure_Isolated(t *testing.T) {
	complexYAML := `
app:
  name: complex-app
  port: 8080
  debug: true
db:
  host: complex-db.local
  port: 5432
  name: complex_db
timeout: 45s
version: "2.1.0"
`

	baseFile := createTempYAML(t, complexYAML)

	// Test with environment overrides for nested values
	setEnvVar(t, "COMPLEX_APP__DEBUG", "false")
	setEnvVar(t, "COMPLEX_DB__PORT", "3306")
	setEnvVar(t, "COMPLEX_TIMEOUT", "60s")

	var cfg TestConfig
	err := LoadConfig(LoaderOptions{
		BaseFile:  baseFile,
		EnvPrefix: "COMPLEX_",
		Delimiter: "__",
		Target:    &cfg,
	})

	require.NoError(t, err)
	assert.Equal(t, "complex-app", cfg.App.Name)
	assert.Equal(t, 8080, cfg.App.Port)
	assert.False(t, cfg.App.Debug) // Overridden by env
	assert.Equal(t, "complex-db.local", cfg.DB.Host)
	assert.Equal(t, 3306, cfg.DB.Port) // Overridden by env
	assert.Equal(t, "complex_db", cfg.DB.Name)
	assert.Equal(t, 60*time.Second, cfg.Timeout) // Overridden by env
	assert.Equal(t, "2.1.0", cfg.Version)
}
